...
client-side (no server): **https://claude.ai/code/artifact/ceec7cc0-6284-4ca7-b495-718b45ad84a7**

It reads `web/model_export.json` (checked in, generated by
`export_web_demo_data.py`) — the same scaler + logistic-regression weights
`predict.py` uses, so the two always agree. After retraining on new data, run:
